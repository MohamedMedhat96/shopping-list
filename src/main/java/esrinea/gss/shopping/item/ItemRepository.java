...
public class ItemRepository {
	...
	public List<ItemModel> getAllItems(Session session) {
		...
		List<ItemModel> currentItems = session.createNativeQuery(
				...
				ItemModel.class).list(); // Returning all items that are not deleted and their categories are also not
											// deleted
	...
	public void addItem(ItemModel item, Session session) throws IncorrectInputException {
		...
		List<ItemModel> items = session
				.createNativeQuery("select * from item where item.item_name = \'" + item.getName() + "\'").list();
		if (items.size() > 0)
	 ...
	 */
	public void editItem(CategoryModel category, int id, String name, String description, Session session)
			throws ItemNotFoundException {
		...
		try {
			...
			int size =session.createNativeQuery(
					"select * from item where item.item_id != "+currentItem.getId()+" and item.item_name = '" + name+"'").list().size();
			if(size > 0)
